...
const MultipageService = ({
  ...
  const [serviceDetailsLoading, setServiceDetailsLoading] = useState(true);  
  ...
  useEffect(() => {
    const fetchCompanyServiceDetails = async () => {
    try {
        const response = await service?.getSliderDetails(slug);
        setServiceDetails(response.data.results );
    } catch (err) {
        setServiceDetailsError(err);
    } finally {
                        ...
                        <div className="row">
                            ...
                            {!replacedMultipage? <Loading/> :
                                ...
                                <h1 style={{textAlign:"left"}}>
                                  ...
                                  {replacedMultipage?.features?.map((feature) => <li key={feature.id}>{feature.feature}</li>) || []}
                                  </ul>
  ...
  <div className="container">
    <div className="row">
      ...
      <section className="multi_serv_slider">        
        <DetailPageSlider serviceDetails={replacedMultipage?.slider_services || serviceDetails} error={serviceDetailsError} loading={serviceDetailsLoading} company={currentCompany}/>    
      </section>  
