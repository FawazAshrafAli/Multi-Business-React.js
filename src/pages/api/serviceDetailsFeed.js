...
export default async function handler(req, res) {
    ...
    const detailPagesRes = await service.getDetails(slug);
  ...
  const feed = new Feed({
    title: `Services - ${currentCompany?.meta_title || currentCompany?.name || ""} - RSS Feed`,
    description: `List of services offered by ${currentCompany?.name}`,
    id: `${siteUrl}/${currentCompany?.slug}/services`,
    link: `${siteUrl}/${currentCompany?.slug}/services`,
    language: 'en',
    ...
    feedLinks: {
      rss2: `${siteUrl}/${currentCompany?.slug}/services/rss`,
    },
    author: {
      name: 'BZ India',
      link: siteUrl,
  ...
  detailPages?.forEach((detail) => {
    feed.addItem({
      title: detail.meta_title || detail.service?.name,
      id: `${siteUrl}/${slug}/${detail?.service?.category_slug}/${detail?.service?.sub_category_slug}/${detail?.slug}`,
      link: `${siteUrl}/${slug}/${detail?.service?.category_slug}/${detail?.service?.sub_category_slug}/${detail?.slug}`,
      description: detail.meta_description || detail.service?.meta_description || "",
      date: new Date(detail.updated || Date.now()),

        enclosure: detail.service?.image_url ? {
            url: `${detail.service.image_url}`,  
            type: getImageMimeType(detail.service.image_url)
        } : undefined
    });
  });
