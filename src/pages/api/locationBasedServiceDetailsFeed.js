...
export default async function handler(req, res) {    
    ...
    const subCategoryRes = await service.getSubCategories("all", undefined, `location_slug=${passingSubCategorySlug}`);
  ...
  if (isListProductDetailsPage) {
    ...
    id: `${siteUrl}/${locationData?.district_slug || locationData?.state_slug}/more-services/${subCategory?.locationSlug || subCategory?.slug}-${locationData?.slug}`,
    link: `${siteUrl}/${locationData?.district_slug || locationData?.state_slug}/more-services/${subCategory?.locationSlug || subCategory?.slug}-${locationData?.slug}`,    
    language: 'en',
    ...
    feedLinks: {
      rss2: `${siteUrl}/${locationData?.district_slug || locationData?.state_slug}/more-services/${subCategory?.locationSlug || subCategory?.slug}-${locationData?.slug}/feed`,
    },
    author: {
      name: 'BZ India',
      link: siteUrl,
